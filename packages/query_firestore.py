...
FIRESTORE_PHARMACIES_DB = config[env]["firestore"]["pharmacies_db"] 
...
def db_geo_query(db, search_geohashes, precision):
    ...
    for geohash in search_geohashes:
        try:
            ...
            query_ref = db.collection(FIRESTORE_PHARMACIES_DB).where(f'location.geohash_{str(precision)}', '==', geohash)
